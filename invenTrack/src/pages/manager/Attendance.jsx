  ...
  useEffect(() => {
    const fetchManagerAndStaff = async () => {
      ...
      try {
        ...
        // First, get the manager's profile to get their branch_id
        const profileResponse = await axios.get("http://localhost:5000/api/auth/profile", {
          headers: { Authorization: `Bearer ${token}` },
        });
        
        const managerBranchId = profileResponse.data.branch_id;
        setBranchId(managerBranchId);

        if (!managerBranchId) {
          setError("Manager is not assigned to any branch.");
          setLoading(false);
          return;
        }

        // Then fetch staff for that branch
        const staffResponse = await axios.get(
          `http://localhost:5000/api/staff/${managerBranchId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        // Filter out the manager themselves if needed (optional)
        // const filteredStaff = staffData.filter(staff => staff._id !== profileResponse.data._id);
        
        setStaffList(staffData);

        // Initialize attendance with all staff as "Present"
        const initialAttendance = staffData.reduce((acc, staff) => {
          acc[staff._id] = "Present";
          return acc;
        }, {});
        ...
        if (err.response?.status === 400) {
          setError("Invalid request. Please check your branch assignment.");
