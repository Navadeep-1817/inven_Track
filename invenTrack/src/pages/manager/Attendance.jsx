...
function Attendance() {
  ...
  // ✅ No need for hardcoded branchId — backend gets it from the logged-in manager (req.user.branch_id)
  // So we’ll fetch staff via token-authenticated request instead.
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      ...
      try {
        ...
        // Initialize attendance state to 'Present'
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Build attendance data as expected by backend
    const attendanceData = {
      staff: Object.keys(attendance).map((staffId) => {
        const staffMember = staffList.find((s) => s._id === staffId);
        return {
              ...
              <tr key={staff._id}>
                ...
                <td>
                  <select
                    ...
                    onChange={(e) =>
                      handleStatusChange(staff._id, e.target.value)
                    }
                  >
                    <option value="Present">Present</option>
                    <option value="Absent">Absent</option>
